...
import json
from app.common.constants import LOCAL_ACCOUNTS_PATH

def get_username() -> str:
    """Gets an available account username from JSON file"""
    with open(LOCAL_ACCOUNTS_PATH, 'r') as f:
        data = json.load(f)
...
def set_account_as_leveled() -> None:
    ...
    for account in data['accounts']:
        if not account['leveled']:
            ...
            with open(LOCAL_ACCOUNTS_PATH, 'w') as json_file:
...
def add_account(account) -> None:
    ...
    with open(LOCAL_ACCOUNTS_PATH, 'w') as outfile:
